<!-- generated from data/. Do not hand-edit; run tools/build_pages.py -->

# Xiaomi Mi Electric Scooter 3

Aftermarket code: `Mi 3` · Wheel: 8.5" · Motor: 300 W · Weight: ~13.2 kg · Released: 2021 · Status: current

Also sold as: Mi Scooter 3, MIJIA Electric Scooter 3, DDHBC02NEB, DDHBC16NEB

## Replacement tires

| Tire | OEM | Size (ETRTO) | Type | Provenance | Measured? |
|---|---|---|---|---|---|
| [Xiaomi M365 OEM 8.5x2 pneumatic](../tires/xiaomi-m365-oem-8.5x2-pneumatic.md) | OEM | 50-134 | pneumatic | Marketplace listing | No |
| [Generic 8.5 inch scooter tire](../tires/generic-8.5x2-pneumatic-pack.md) | Generic | 50-134 (claimed) | pneumatic | Marketplace listing | No |
| [Generic 8.5 inch honeycomb tire](../tires/generic-8.5-honeycomb-solid.md) | Generic | n/a (imperial 8.5) | solid honeycomb | Marketplace listing | No |

## Axle / wheel mount: Xiaomi M365 family 8.5-inch wheel

Provenance: **Inferred**

Status: **unmeasured**

8.5-inch wheel assembly shared across Xiaomi's first-generation cluster:
M365, M365 Pro, 1S, Essential, Pro 2, and the Mi Electric Scooter 3.
Standard tire is 8.5x2 (ETRTO 50-134) pneumatic on a split rim, with a
Schrader valve. Aftermarket also offers solid and honeycomb (semi-solid)
tires marketed for the same hub. Marketplace listings cluster on the
scooter model name ("M365", "Pro 2", "1S") rather than on an engineering
SKU. Cross-fit to non-Xiaomi 8-inch scooters (e.g. Ninebot ES series 8")
is community-reported, not manufacturer-claimed, and varies by rim
geometry; treat as a separate slot until measured.

## Brake pads

| Brake pad | OEM | Caliper mount | Compound | Provenance | Measured? |
|---|---|---|---|---|---|
| [Generic Mi Electric Scooter 3 brake pads](../pads/generic-mi-3-dual-pad-disc-pad-pack.md) | Generic | dual-pad rear disc, Xiaomi Scooter 3 stock caliper | resin / organic (claimed; varies) | Marketplace listing | No |

## Caliper mount: Xiaomi Mi Electric Scooter 3 rear dual-pad disc caliper

Provenance: **Manufacturer claim**

Status: **unmeasured**

Rear dual-pad disc brake used on the Mi Electric Scooter 3. Xiaomi's
global product page states "rear dual-pad disc brake" and says the brake
prolongs pad service life. This profile is kept separate from the M365
rear caliper until pad outline and retention are measured or otherwise
verified.

## Inner tubes

| Tube | OEM | Size | Valve | Provenance | Measured? |
|---|---|---|---|---|---|
| [Xiaomi Electric Scooter Inner Tube 8.5](../tubes/xiaomi-m365-oem-8.5x2-inner-tube.md) | OEM | 8.5x2 | Schrader | Marketplace listing | No |
| [Generic 8.5 inch scooter inner tube](../tubes/generic-8.5x2-schrader-inner-tube.md) | Generic | 8.5x2 | Schrader | Marketplace listing | No |

## Replacement grip tape

| Grip tape | OEM | Deck length (mm) | Cutouts | Provenance | Measured? |
|---|---|---|---|---|---|
| [Generic M365 adhesive non-slip footrest coating](../grip-tape/generic-m365-family-560x132-foot-mat.md) | Generic | 560 | seller-stated pre-cut M365-family footrest coating; exact cutouts unverified | Marketplace listing | No |

## Deck profile: Xiaomi M365 / 1S / Essential / Mi 3 deck mat

Provenance: **Marketplace listing**

Status: **unmeasured**

Working deck-mat profile for the Xiaomi M365-family short deck. A
50factory listing for an eWheel adhesive non-slip footrest coating gives
stated mat dimensions of 560 mm x 132 mm and lists Xiaomi M365, 1S,
Essential, and Mi 3 compatibility. Treat the Mi 3 assignment as a
marketplace claim: other aftermarket sellers sometimes split M365/1S and
Pro/Pro 2 mats, and this pass did not physically verify the Mi 3 top-deck
outline or cutout pattern.

## Notes

The Mi Electric Scooter 3 keeps the 8.5x2 pneumatic format established by
the M365 and shares the family slot for tire compatibility per
aftermarket sellers. Xiaomi does not publish a hub-side spec sheet that
would let the project assert manufacturer-claim cross-fit, so the
interface assignment is inferred from the consistent 8.5x2 dimension and
the absence of any manufacturer-stated incompatibility. Xiaomi's global
product page identifies a rear dual-pad disc brake, so the pad-bearing
brake architecture is manufacturer-claim for the global DDHBC16NEB page.
DDHBC02NEB is retained as a regional alias from the earlier scooter data
pass. Motor power and weight figures are manufacturer claims. Grip-tape
fitment is left at marketplace-claim because this pass found a seller
cross-listing Mi 3 with M365 / 1S / Essential mats, but no measured mat
outline or Xiaomi spare-part listing.

## Sources

- <https://www.mi.com/global/product/mi-electric-scooter-3/>
