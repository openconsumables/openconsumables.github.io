---
source_yaml: data/openscoot/units/xiaomi-mi-electric-scooter-3.yml
---

<!-- generated from data/. Do not hand-edit; run tools/build_pages.py -->

# Xiaomi Mi Electric Scooter 3

Model / fitment code: `Mi 3` · Wheel: 8.5" · Motor: 300 W · Weight: ~13.2 kg · Released: 2021 · Status: current

Also sold as: Mi Scooter 3, MIJIA Electric Scooter 3, DDHBC02NEB, DDHBC16NEB

## Caliper mount: Xiaomi Mi Electric Scooter 3 rear dual-pad disc caliper

Provenance: **Manufacturer claim**

Status: **unmeasured**

Rear dual-pad disc brake used on the Mi Electric Scooter 3. Xiaomi's
global product page states "rear dual-pad disc brake" and says the brake
prolongs pad service life. This profile is kept separate from the M365
rear caliper until pad outline and retention are measured or otherwise
verified.

## Replacement grip tape

| Grip tape | OEM | Deck length (mm) | Cutouts | Provenance | Measured? | Source |
|---|---|---|---|---|---|---|
| [Generic M365 adhesive non-slip footrest coating](../grip-tape/generic-m365-family-560x132-foot-mat.md) | Generic | 560 | seller-stated pre-cut M365-family footrest coating; exact cutouts unverified | Marketplace listing | No | [Source](<https://en.50factory.com/parts%20news/182218-xiaomi-m365-ewheel-doodle-scooter-footboard-cover.html>) |

## Deck profile: Xiaomi M365 / 1S / Essential / Mi 3 deck mat

Provenance: **Marketplace listing**

Status: **unmeasured**

Working deck-mat profile for the Xiaomi M365-family short deck. A
50factory listing for an eWheel adhesive non-slip footrest coating gives
stated mat dimensions of 560 mm x 132 mm and lists Xiaomi M365, 1S,
Essential, and Mi 3 compatibility. Treat the Mi 3 assignment as a
marketplace claim: other aftermarket sellers sometimes split M365/1S and
Pro/Pro 2 mats, and this pass did not physically verify the Mi 3 top-deck
outline or cutout pattern.

## Notes

Xiaomi identifies a rear dual-pad disc brake on Mi Electric Scooter 3. The project has not established an exact replacement pad, tire or tube part number for this device. Nominal 8.5-inch wheels do not prove cross-fit to M365. The retained deck-mat claim is a seller claim, not a measured outline.

## Sources

- <https://www.mi.com/global/product/mi-electric-scooter-3/>

## Corrections and additions

[Suggest a correction or addition](https://github.com/openconsumables/openconsumables.github.io/issues/new?template=evidence-report.md&title=Reference%3A+xiaomi-mi-electric-scooter-3&body=Device%2Fpart+or+page%3A+openscoot%2Funits%2Fxiaomi-mi-electric-scooter-3+%28https%3A%2F%2Fopenconsumables.org%2Fcategories%2Fopenscoot%2Funits%2Fxiaomi-mi-electric-scooter-3%2F%29%0ACorrection+or+addition%3A+%0ASource%3A+%0AUncertainty%3A+%0A) (GitHub sign-in required). One sourced fact helps; better sources and conflicting fit evidence are welcome. For a broken link or an unverified claim, just identify the problem.

[Copy a report without GitHub](../../../contributing.md#send-a-short-report) or [inspect the source YAML](https://github.com/openconsumables/openconsumables.github.io/blob/master/data/openscoot/units/xiaomi-mi-electric-scooter-3.yml).
